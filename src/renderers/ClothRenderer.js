...
export class ClothRenderer extends RendererInterface {
  ...
  update(motionPoints, dt, state, rainbowHue) {
    ...
    const springLerpFactor = 1 - Math.pow(0.88, 60 * dt);
    const dampingFactor = Math.pow(0.88, 60 * dt);
    const diffusionFactor = 1 - Math.pow(0.65, 60 * dt);

    for (let c = 0; c <= cols; c++) {
      for (let r = 0; r <= rows; r++) {
        const node = this.clothGrid[c][r];

        node.z += (node.zTarget - node.z) * springLerpFactor;
        node.zTarget *= dampingFactor;
        ...
        if (c > 0 && c < cols && r > 0 && r < rows) {
          ...
          node.z += (avg - node.z) * diffusionFactor;
        }
